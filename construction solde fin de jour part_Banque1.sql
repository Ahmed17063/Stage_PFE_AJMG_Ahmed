...
FROM tj81
where date_valeur>'2021-03-31' and date_valeur < '2021-07-01' and COPRO in (159, 160, 196)
...
drop table if exists mvts_jours_part;
CREATE TABLE `mvts_jours_part` (
...
create index idx_cpte on mvts_jours_part (compte, date_val);
create index idx_cpte2 on mvts_jours_part (compte, id);
create index idx_cpte3 on mvts_jours_part (compte);

INSERT INTO mvts_jours_part (compte, COPRO, date_val, summontant)
SELECT compte, COPRO, date_val, summontant
FROM inter_1 
order by compte, date_val desc;

-- update solde fin de trimestre
update mvts_jours_part
set SFM = (select max(MSVAFP)
		   from tq2i a
		   where nucoi=compte);

-- update NBJ
UPDATE mvts_jours_part as A   
INNER JOIN mvts_jours_part as B 
...
drop table if exists mvts_jours_part2;
Create table mvts_jours_part2 like mvts_jours_part;
-- 5400 sec
call calcul_sfj_part(); -- la procédure qui remplit la table comme il se doit.
...
update `synthese_decouvert_part`
set `NBJCdec max` = 0
...
Select count(*) from synthese_decouvert_part where `NBJ dec trim` between  QCNJDB-3 and QCNJDB+3 and  `NBJ dec trim`>0;
Select count(*) from synthese_decouvert_part where `NBJ dec trim` not between  QCNJDB-3 and QCNJDB+3 and  `NBJ dec trim`>0;
Select count(*) from synthese_decouvert_part where `NBJ dec trim` = 0; 

Select * from synthese_decouvert_part where `NBJ dec trim` <> QCNJDB and  `NBJ dec trim`>0;
